...
async Task AddAgent(string firstname, string lastname, string email, string password)
{
    ...
    await using var checkCmd = db.CreateCommand("SELECT COUNT(*) FROM agents WHERE email = @email");
    checkCmd.Parameters.AddWithValue("@email", email);
    var emailExists = (long)await checkCmd.ExecuteScalarAsync() > 0;
    ...
    await using var insertCmd = db.CreateCommand(
        "INSERT INTO agents (firstname, lastname, email, password) VALUES (@firstname, @lastname, @email, @password)");
